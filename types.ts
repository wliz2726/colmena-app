...
/**
 * Custom fields para clientes
 * Se obtienen via GetClientsDetails.customfields
 */
...
export interface Condominimos {
  ...
  address1?: string;
  address2?: string; // Nombre del condominio
  city?: string;
  state?: string;
  postcode?: string;
  country?: string;
...
/**
 * Datos extendidos de un condómino
 * Se obtiene via GetClientsDetails
 */
...
export interface InvoiceTransaction {
  ...
  amount: string;
  fee: string;
  date: string; // YYYY-MM-DD HH:ii:ss
  gateway: string;
  type: 'payment' | 'refund';
}
...
export interface Invoice {
  ...
  datepaid: string; // YYYY-MM-DD HH:ii:ss or "0000-00-00 00:00:00"
  ...
  total: string;
  credit: string;
  notes: string; // Descripción del servicio: "Mantenimiento común - Junio 2026"
